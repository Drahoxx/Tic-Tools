...
def _import(plugin,argv):
    try:
        ...
        import_module('plugins.' + plugin).main(argv)
    except:
        print(f"\x1b[31m\x1b[1m/!\\\x1b[0m ERROR : {plugin} is not a plugin",file=stderr)
args = {}

if len(argv) < 2 :
    print("\x1b[31m\x1b[1m/!\\\x1b[0m ERROR : Missing Argument\n",file=stderr)
    exit()
